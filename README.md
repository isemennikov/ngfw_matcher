...
Дополнительные режимы: поиск правил по имени/UUID (`find-rule`),
полный просмотр всех правил для заданного src (`--fullview`),
математический анализ теневых правил без задания трафика (`check-shadowed`).
...
python -m ngfw_match --host https://localhost:3663 --user admin --pass secret \

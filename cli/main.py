...
if TYPE_CHECKING:
    pass
from .output import print_result, print_summary, export_csv, export_fullview_json, c, _C
        ...
        for dst in dsts:
            for port in ports:
                flows.append(TrafficFlow(
                    ...
                    dst_port = port,
                    protocol = proto,
                    src_port = getattr(args, "sport", 0) or 0,
                    zone_src = getattr(args, "zone_src", "") or "",
                    zone_dst = getattr(args, "zone_dst", "") or "",
                ))
    return flows


def _run_fullview(args, matcher: RuleMatcher):
    """--fullview: все правила по src/dst/port (не first-match), выход в JSON."""
    output_path = getattr(args, "output", None)
    if not output_path:
        die("--fullview требует --output FILE.json")

    if not getattr(args, "proto", None):
        args.proto = "any"
    ...
    export_fullview_json(
        flow_results, query, output_path,
        fetch_net_group=matcher.resolver._fetch_net_group,
        fetch_svc_group=matcher.resolver._fetch_svc_group,
    )
...
def _run_flow(args, matcher: RuleMatcher):
    ...
    if getattr(args, "fullview", False):
        _run_fullview(args, matcher)
        return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    m.add_argument("--sport", metavar="PORT", type=int, default=0)
    m.add_argument("--zone-src", metavar="ZONE")
    m.add_argument("--zone-dst", metavar="ZONE")

    m.add_argument("--fullview", action="store_true",
                   help=("Показать все совпадающие правила по --src (не first-match).\n"
                         "Требует --output FILE.json. Выводит полные поля dst/service/app."))
